...
import MuaAnalysis from '../../views/mua/muaAnalysis.vue';
...
// 组件引用
const muaAnalysisRef = ref<InstanceType<typeof MuaAnalysis>>();

// 从父组件注入的刷新回调函数注册器
const registerRefreshAnalysisCallback = inject<((callback: () => void) => void) | null>(
	'registerRefreshAnalysisCallback',
	null
);

// 组件挂载后，注册刷新回调函数
onMounted(() => {
	if (registerRefreshAnalysisCallback && muaAnalysisRef.value) {
		registerRefreshAnalysisCallback(() => {
			if (muaAnalysisRef.value?.refreshMuaAnalysis) {
				muaAnalysisRef.value.refreshMuaAnalysis();
			}
		});
	}
});
