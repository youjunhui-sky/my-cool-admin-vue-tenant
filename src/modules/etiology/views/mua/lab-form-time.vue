		...
		<cl-crud ref="Crud">
			<cl-row>
				<cl-table ref="Table" :data="tableData">
					...
					<!-- 自定义检测时间列的渲染 -->
					<template #column-testTime="{ scope }">
						<el-date-picker
							...
							@change="val => handleTestTimeChange(scope.row, val)"
						/>
					</template>
					<!-- 自定义结果值列表头，添加操作按钮 -->
 ...
 * 处理检测时间变化
 */
const handleTestTimeChange = async (row, newValue) => {
	//console.log('检测时间变化:', {
	// 	itemName: row.itemName,
	// 	oldTime: row.testTime,
	// 	newTime: newValue
	// });

	// 更新行数据中的检测时间
	row.testTime = newValue;

	// 如果有结果值，则保存数据
	if (row.resultValue1) {
		await saveMuaItems(row, 1);
	}
};
