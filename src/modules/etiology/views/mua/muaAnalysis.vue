				...
				<div ref="lowerSectionRef" class="lower-section">
					<div class="lower-content">
						...
						<div class="tables-row">
							<!-- Table 1 with Analysis Button -->
							<div class="table-group">
								<div class="table-container">
									<div class="table-title-row">
										<div class="table-title">检验结果详情</div>
									...
									>
										...
										>
											<template #default="{ row }">
												<div class="tags-container">
													<el-tag
														v-for="tageValue in formatDescription(
															row.description
														)"
														:key="tageValue"
														:type="
															hasArrowUp(row.description) ||
															hasArrowDown(row.description)
																? hasArrowUp(row.description)
																	? 'danger'
																	: 'primary'
																: 'info'
												...
												<!-- <span
													v-html="formatDescription(row.description)"
												></span> -->
											</template>
										</el-table-column>
										<el-table-column
										...
										<template #default="{ row }">
											<el-button
												...
												type="primary"
												size="small"
												@click="handleRowMoveUpResult(row)"
												:disabled="getRowIndexResult(row) === 0"
											>
												上移
											</el-button>
											<el-button
												...
												type="primary"
												size="small"
												@click="handleRowMoveDownResult(row)"
												:disabled="
													getRowData1.length - 1
												"
											>
												下移
											</el-button>
							...
							<div class="table-group">
								<div class="table-container">
									<div class="table-title">诊断信息</div>
									<!-- 诊断卡片列表 -->
									<div class="diagnostic-cards-container">
										<div
											...
											:class="{ selected: isDiagnosticSelected(item) }"
											@click="toggleDiagnosticSelection(item)"
										>
											<div class="diagnostic-card-content">
												<div class="diagnostic-header-row">
													<div class="diagnostic-code-label">
														诊断编码
													</div>
													<div class="diagnostic-name-label">
														诊断名称
													</div>
												</div>
									...
									<div class="table-buttons-bottom">
										<div class="buttons-row">
											<el-button
												...
												size="small"
												@click="onMoveUp"
												:disabled="!hasSelectedRow || !canMoveUp"
												>上移</el-button
											>
											<el-button
												type="primary"
												size="small"
												@click="onMoveDown"
												:disabled="!hasSelectedRow || !canMoveDown"
												>下移</el-button
											>
...
import PrintContent from './components/mua-overall-print-content.vue';
import MuaHistory from './components/mua-history.vue';
...
const hasSelectedRow = computed(() => selectedDiagnosticItems.value.length > 0);
const table2Ref = ref();

// 诊断卡片选择相关
const selectedDiagnosticItems = ref<DiagnosticData[]>([]);

// 判断诊断项是否被选中
const isDiagnosticSelected = (item: DiagnosticData) => {
	return selectedDiagnosticItems.value.some(
		selected => selected.diagnosticCode === item.diagnosticCode
	);
};

// 获取选中卡片的索引
const getSelectedDiagnosticIndex = computed(() => {
	if (selectedDiagnosticItems.value.length !== 1) {
		return -1;
	}
	const selectedItem = selectedDiagnosticItems.value[0];
	return tableData2.value.findIndex(item => item.diagnosticCode === selectedItem.diagnosticCode);
});

// 判断是否可以上移
const canMoveUp = computed(() => {
	const index = getSelectedDiagnosticIndex.value;
	return index > 0;
});
...
const canMoveDown = computed(() => {
	const index = getSelectedDiagnosticIndex.value;
	return index >= 0 && index < tableData2.value.length - 1;
});

// 切换诊断项选择状态（只能选中一个）
const toggleDiagnosticSelection = (item: DiagnosticData) => {
	const index = selectedDiagnosticItems.value.findIndex(
		selected => selected.diagnosticCode === item.diagnosticCode
	);
	if (index > -1) {
		// 如果已选中，则取消选中
		selectedDiagnosticItems.value.splice(index, 1);
		selectedRows.value = selectedRows.value.filter(
			row => row.diagnosticCode !== item.diagnosticCode
		);
	} else {
		// 如果未选中，先清空之前的选中，再选中当前项（只能选中一个）
		selectedDiagnosticItems.value = [item];
		selectedRows.value = [item];
	}
};

// 诊断卡片上移
const handleDiagnosticMoveUp = async (item: DiagnosticData, index: number) => {
	if (index <= 0) {
	...
	// 交换位置
	const temp = tableData2.value[index];
	tableData2.value[index] = tableData2.value[index - 1];
	tableData2.value[index - 1] = temp;
	...
	try {
		...
		// 如果数据库更新失败，回滚前端操作
		const temp = tableData2.value[index - 1];
		tableData2.value[index - 1] = tableData2.value[index];
		tableData2.value[index] = temp;
...
// 诊断卡片下移
const handleDiagnosticMoveDown = async (item: DiagnosticData, index: number) => {
	if (index >= tableData2.value.length - 1) {
...
const onSave = async (isSave: boolean) => {
	...
	try {
		...
		if (isSave) {
			ElMessage.success('诊疗信息保存成功');
		}
	} catch (error: any) {
		console.error('保存诊疗信息失败:', error);
		ElMessage.error('保存诊疗信息失败: ' + (error.message || '未知错误'));
	}
};
...
//刷新
const refreshMuaAnalysis = async () => {
	await initMua();
	await initMuaResult();
	await initMuaContent();
	await initMuaAnalysis();
...
const onTreatmentSuggestion = async () => {
	...
	try {
		...
		if (diagFlag) {
			ElMessage.success('当前无诊断结果，请完善数据集');
			return;
		}
		// 设置加载状态
		...
		console.error('诊疗建议失败:', error);
		ElMessage.error('诊疗建议失败: ' + (error.message || '未知错误'));
	} finally {
		// 无论成功或失败，都要隐藏进度条
...
const onMoveUp = async () => {
	if (selectedDiagnosticItems.value.length !== 1) {
		ElMessage.warning('请选择一个诊断进行上移操作');
		return;
	}

	const selectedRow = selectedDiagnosticItems.value[0];
	const currentIndex = tableData2.value.findIndex(
		item => item.diagnosticCode === selectedRow.diagnosticCode
	);
	...
	try {
		await updateDatabaseOrder();
		// 更新选中项，保持选中状态
		selectedDiagnosticItems.value = [tableData2.value[currentIndex - 1]];
		selectedRows.value = [tableData2.value[currentIndex - 1]];
		...
		console.error('上移失败:', error);
		ElMessage.error('上移失败: ' + (error instanceof Error ? error.message : '未知错误'));
	}
};
...
const onMoveDown = async () => {
	if (selectedDiagnosticItems.value.length !== 1) {
		ElMessage.warning('请选择一个诊断进行下移操作');
		return;
	}
	...
	try {
		...
		console.error('下移失败:', error);
		ElMessage.error('下移失败: ' + (error instanceof Error ? error.message : '未知错误'));
	}
};
...
const onDelete = async () => {
	if (selectedDiagnosticItems.value.length === 0) {
	...
	try {
		await ElMessageBox.confirm(
			`确定要删除选中的 ${selectedDiagnosticItems.value.length} 条诊断数据吗？删除后不可恢复。`,
			'删除确认',
			{
	...
	const originalSelection = [...selectedDiagnosticItems.value];

	// 从后往前删除，避免索引变化问题
	const indicesToDelete = selectedDiagnosticItems.value
		.map(row => tableData2.value.findIndex(item => item.diagnosticCode === row.diagnosticCode))
		.sort((a, b) => b - a);
	...
	try {
		...
		selectedDiagnosticItems.value = originalSelection;
...
const formatDescription = (description: string | undefined | null): string[] => {
	...
	const tageValue = description.split(';');

	const filteredArr = tageValue.filter(item => {
...
// 获取行的索引
const getRowIndexResult = (row: any) => {
	return tableData1.value.findIndex(item => item === row);
};

// 单行上移结果
const handleRowMoveUpResult = async (row: any) => {
	const currentIndex = getRowIndexResult(row);
	if (currentIndex <= 0) {
...
// 单行下移结果
const handleRowMoveDownResult = async (row: any) => {
	const currentIndex = getRowIndexResult(row);
...
const onMoveUpResult = async () => {
	...
	await handleRowMoveUpResult(selectedRow);
...
const onMoveDownResult = async () => {
	...
	const selectedRow = selectedResultRows.value[0];
	await handleRowMoveDownResult(selectedRow);
					...
					&:nth-child(1) {
						...
						.table-container {
							.el-table {
								flex: 0 0 auto;
								height: auto;
								max-height: 896px; // 表头56px + 15行数据840px = 896px
								overflow: hidden;
								order: 1; // 确保表格在按钮上方
								...
								:deep(.el-table__body-wrapper) {
									max-height: 840px; // 15行数据的高度（56px * 15）
									overflow-y: auto;
								}

								// 控制单元格行高
								:deep(.el-table__cell) {
									padding: 8px 0px; // 增加单元格内边距
								}
							...
							.treatment-suggestion-area {
								...
								.details-content {
									...
									background-color: rgb(
										240 253 244 / var(--tw-bg-opacity, 1)
									) !important;
								}
							}
						}
