<template>
	<div class="lab-form-wrapper">
		<div>
			...
			<p hidden>parentGroupData: {{ JSON.stringify(parentGroupData) }}</p>
		</div>
		<cl-crud ref="Crud">
			<cl-row>
		...
		<lab-item-binding-form
			...
			:name="props.name"
			:outpatient-no="props.outpatientNo"
			:inpatient-no="props.inpatientNo"
...
import { analyzeResultValueMany } from '/$/swl/utils/labResultAnalysis';
...
const Table = useTable({
	...
	props: {
		...
		rowClassName: ({ row }) => {
			...
			return row.qualitativeResult1 === '↓' || row.qualitativeResult1 === '↑'
				? 'abnormal-row'
				: '';
		}
	},
...
const handleBindButtonClick = async (index: number) => {
	//判断outpatientNo、inpatientNo是否为空
	if (
		(props.outpatientNo == null || props.outpatientNo == '') &&
		(props.inpatientNo == null || props.inpatientNo == '')
	) {
		ElMessage.error('门诊号或住院号为空，请先绑定');
		return;
	}
...
/**
 * 处理表头按钮点击事件
 * @param command 命令类型：clearAll-批量清空, validateAll-批量校验, fillDefault-填充默认值
 */
const handleHeaderButtonClick = async (command: string) => {
	switch (command) {
		case 'clearAll':
			// 批量清空所有结果值
			try {
		...
		case 'validateAll':
			...
			try {
				...
				tableData.value.forEach(row => {
					if (row.resultValue1) {
						const updatedRow = analyzeResultValueMany(row as any, row.resultValue1, 1);
						Object.assign(row, updatedRow);

						if (row.qualitativeResult1 === '↓' || row.qualitativeResult1 === '↑') {
							hasAbnormal = true;
							abnormalItems.push(row.itemName);
						}
					}
				});
				...
				if (hasAbnormal) {
					ElMessage.warning(
						`发现异常值：${abnormalItems.slice(0, 5).join('、')}${
							abnormalItems.length > 5 ? '...' : ''
						}`
					);
				} else {
					ElMessage.success('所有结果值校验通过');
				}
			} catch (error) {
		...
		case 'fillDefault':
			...
			try {
				let filledCount = 0;
				tableData.value.forEach(row => {
					// 如果结果值为空且有参考范围，可以填充参考范围的中值
					if (!row.resultValue1 && row.referenceRange) {
						const rangeMatch = row.referenceRange.match(
							/(\d+\.?\d*)\s*[-~]\s*(\d+\.?\d*)/
						);
						if (rangeMatch && rangeMatch.length >= 3) {
							const min = parseFloat(rangeMatch[1]);
							const max = parseFloat(rangeMatch[2]);
							const midValue = ((min + max) / 2).toFixed(2);
							row.resultValue1 = midValue;
							filledCount++;

							// 自动分析并更新异常提示
							const updatedRow = analyzeResultValueMany(row as any, midValue, 1);
							Object.assign(row, updatedRow);
						}
					}
				});
				...
				if (filledCount > 0) {
					ElMessage.success(`已填充 ${filledCount} 个默认值`);
				} else {
					ElMessage.info('没有可填充的默认值');
				}
			} catch (error) {
				console.error('填充默认值失败:', error);
				ElMessage.error('填充默认值失败');
			}
			break;
