<template>
	<cl-crud ref="Crud">
		...
		<cl-upsert ref="Upsert">
			<template #slot-patient-history-questionnaire>
...
defineOptions({
	name: 'history-questionnaire-info'
});
...
const Crud = useCrud(
	{
		service: service.patient.historyQuestionnaire
	},
	app => {
		app.refresh({
			size: 20 // 默认每页显示20条
...
const Upsert = useUpsert({
	...
	items: [
		{
			...
			component: {
				name: 'slot-patient-history-questionnaire'
			}
		},
		{
			prop: 'id',
	...
	onSubmit(data, { next, done }) {
		...
		// 验证表单
		const validateResult = questionnaireInfoRef.value.validate();
		if (validateResult && !validateResult.value.valid) {
			ElMessage.error(validateResult.value.message);
			done();
			return;
		}
		...
		const formData = questionnaireInfoRef.value.form || {};
		const submitData = {
			...formData,
			...data // 合并 cl-upsert 的数据（如 id）
		};
