<template>
	<el-scrollbar>
		<div class="demo-home">
			...
			<el-row :gutter="10">
				<el-col :lg="24" :xs="24">
					<tab-chart-curative :curativeData="curativeData" />
				</el-col>
			</el-row>
			<el-row :gutter="10">
				<el-col :lg="24" :xs="24">
					<stat-curative :curativeData="curativeData" />
				</el-col>
			</el-row>
		</div>
	</el-scrollbar>
...
import TabChartCurative from '../components/tab-chart-swl-curative.vue';
import StatCurative from '../components/stat-swl-curative.vue';
...
const curativeData = ref<ItemDataAge[]>([]);
...
async function fetchStatData() {
	try {
		const res = await service.stat.info.getCurativeStat({
			startDate: visitDate.value[0],
			endDate: visitDate.value[1]
		});
		curativeData.value = res;
	} catch (error) {
